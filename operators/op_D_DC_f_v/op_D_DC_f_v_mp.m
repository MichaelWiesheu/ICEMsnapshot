...
function A = op_D_DC_f_v_mp(spv, spg, msh, coeff, patch_list)
    
    if ((spv.npatch~= spg.npatch)  || (spv.npatch ~= msh.npatch))
        error ('op_D_DC_f_v_mp: the number of patches does not coincide')
    end
    ...
    for iptc = patch_list
      ...
      [indices, vs] = op_D_DC_f_v_tp (spv.sp_patch{iptc}, spg.sp_patch{iptc}, msh.msh_patch{iptc}, coeff);
        ...
        if (~isempty (spg.dofs_ornt))
            warning("op_D_DC_f_v_mp: dofs_ornt not tested yet")
