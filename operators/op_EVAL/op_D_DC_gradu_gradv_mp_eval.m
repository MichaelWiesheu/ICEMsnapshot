function A = op_D_DC_gradu_gradv_mp_eval(spu, spuEval, spv, spvEval, spg, spgEval, msh, mshEval, coeff, patch_list)

  inds = cell(msh.npatch, 1);
  vals = cell(msh.npatch, 1);
  
  for iPatch = patch_list
    [ind, v] = op_D_DC_gradu_gradv(spuEval{iPatch}, spvEval{iPatch}, spgEval{iPatch}, mshEval{iPatch}, coeff);
